defmodule Mayfly.Loop do
  ...
  use GenServer
  alias Mayfly.{Runtime, Handler, Error}
  require Logger
  ...
  defp process_invocation(headers, body) do
    case Mayfly.Helpers.get_request_id(headers) do
      nil ->
        Logger.error("Missing request ID in headers")
        :ok
      ...
      req_id ->
        ...
        case decode_payload(body) do
          ...
          {:error, error} ->
            Runtime.invocation_error(req_id, error)
        end
    end
  end
  ...
  defp decode_payload(body) do
    try do
      ...
      e ->
        ...
        {:error, Error.format_error(e, __STACKTRACE__)}
    end
  end
end

defmodule Mayfly.Helpers do
  @moduledoc """
